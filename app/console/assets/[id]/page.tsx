...
import { getAssetById, getIssuesForAsset, getHistoryForAsset } from "@/lib/mock-data";
...
export default function AssetDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const publicUrl =
    (typeof window !== "undefined" ? window.location.origin : "https://maintainiq.app") +
    `/asset/${asset.publicId}`;
  const issues = getIssuesForAsset(asset.id);
  const history = getHistoryForAsset(asset.id);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true">
      <div className="w-full max-w-sm rounded-xl border border-line bg-surface p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-lg p-1 text-muted hover:bg-surface-2 hover:text-foreground">✕</button>
        </div>
