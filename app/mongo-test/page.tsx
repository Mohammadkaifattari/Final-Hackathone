...
export default function MongoTestPage() {
  ...
  const loadNotes = useCallback(async () => {
    ...
    setError("")
    try {
      const res = await fetch("/api/mongo-notes")
  ...
  function startEdit(note: NoteType) {
  ...
  async function deleteNote(noteId: string) {
    setError("")
    try {
      const res = await fetch(`/api/mongo-notes?id=${noteId}`, { method: "DELETE" })
      const data = await res.json()
      if (!data.ok) setError(data.error)
  ...
  if (status === "unauthenticated") {
    return (
      <div className="flex flex-col gap-4">
        <h1 className="text-2xl font-bold">MongoDB Test (CRUD)</h1>
        <p>Please sign in to use MongoDB notes.</p>
        <button className="border px-3 py-1 rounded w-fit" onClick={() => signIn()}>
          Sign in
        </button>
      </div>
    )
                ...
                {editingId === note.id ? (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                      <button className="border px-2 py-1 rounded" onClick={() => saveEdit(note.id)} disabled={busy}>
                        Save
                      </button>
                      <button className="border px-2 py-1 rounded" onClick={() => setEditingId(null)}>
                        Cancel
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between gap-3">
                    ...
                    <div className="flex gap-2 shrink-0">
                      ...
                      <button className="border px-2 py-1 rounded" onClick={() => deleteNote(note.id)}>
                        Delete
                      </button>
                    </div>
                  </div>
